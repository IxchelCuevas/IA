import os
import random
import re
import sys

DAMPING = 0.85
SAMPLES = 10000


def main():
    if len(sys.argv) != 2:
        sys.exit("Usage: python pagerank.py corpus")
    corpus = crawl(sys.argv[1])
    ranks = sample_pagerank(corpus, DAMPING, SAMPLES)
    print(f"PageRank Results from Sampling (n = {SAMPLES})")
    for page in sorted(ranks):
        print(f"  {page}: {ranks[page]:.4f}")
    ranks = iterate_pagerank(corpus, DAMPING)
    print(f"PageRank Results from Iteration")
    for page in sorted(ranks):
        print(f"  {page}: {ranks[page]:.4f}")


def crawl(directory):
    """
    Parse a directory of HTML pages and check for links to other pages.
    Return a dictionary where each key is a page, and values are
    a list of all other pages in the corpus that are linked to by the page.
    """
    pages = dict()

    # Extract all links from HTML files
    for filename in os.listdir(directory):
        if not filename.endswith(".html"):
            continue
        with open(os.path.join(directory, filename)) as f:
            contents = f.read()
            links = re.findall(r"<a\s+(?:[^>]*?)href=\"([^\"]*)\"", contents)
            pages[filename] = set(links) - {filename}

    # Only include links to other pages in the corpus
    for filename in pages:
        pages[filename] = set(
            link for link in pages[filename]
            if link in pages
        )

    return pages


def transition_model(corpus, page, damping_factor):
    """
    Return a probability distribution over which page to visit next,
    given a current page.

    With probability `damping_factor`, choose a link at random
    linked to by `page`. With probability `1 - damping_factor`, choose
    a link at random chosen from all pages in the corpus.
    """

    distribution = dict()
    total_pages = len(corpus)

    # If page has no outgoing links, treat it as linking to every page
    if len(corpus[page]) == 0:
        for p in corpus:
            distribution[p] = 1 / total_pages
        return distribution

    # Base probability: random jump to any page
    for p in corpus:
        distribution[p] = (1 - damping_factor) / total_pages

    # Extra probability: follow one of the links from current page
    linked_pages = corpus[page]
    for linked_page in linked_pages:
        distribution[linked_page] += damping_factor / len(linked_pages)

    return distribution

def sample_pagerank(corpus, damping_factor, n):
    """
    Return PageRank values for each page by sampling `n` pages
    according to transition model, starting with a page at random.

    Return a dictionary where keys are page names, and values are
    their estimated PageRank value (a value between 0 and 1). All
    PageRank values should sum to 1.
    """

    counts = dict()

    for page in corpus:
        counts[page] = 0

    # First sample: choose a page randomly
    current_page = random.choice(list(corpus.keys()))
    counts[current_page] += 1

    # Remaining samples
    for _ in range(n - 1):
        distribution = transition_model(corpus, current_page, damping_factor)

        pages = list(distribution.keys())
        probabilities = list(distribution.values())

        current_page = random.choices(pages, weights=probabilities, k=1)[0]
        counts[current_page] += 1

    # Convert counts into probabilities
    ranks = dict()

    for page in counts:
        ranks[page] = counts[page] / n

    return ranks


def iterate_pagerank(corpus, damping_factor):
    """
    Return PageRank values for each page by iteratively updating
    PageRank values until convergence.

    Return a dictionary where keys are page names, and values are
    their estimated PageRank value (a value between 0 and 1). All
    PageRank values should sum to 1.
    """

    total_pages = len(corpus)

    # Initial PageRank: all pages equally likely
    ranks = dict()

    for page in corpus:
        ranks[page] = 1 / total_pages

    converged = False

    while not converged:
        new_ranks = dict()
        converged = True

        for page in corpus:
            rank = (1 - damping_factor) / total_pages

            for possible_page in corpus:
                links = corpus[possible_page]

                # If a page has no links, treat it as linking to every page
                if len(links) == 0:
                    rank += damping_factor * (ranks[possible_page] / total_pages)

                # If possible_page links to page, add its contribution
                elif page in links:
                    rank += damping_factor * (ranks[possible_page] / len(links))

            new_ranks[page] = rank

            if abs(new_ranks[page] - ranks[page]) > 0.001:
                converged = False

        ranks = new_ranks

    return ranks


if __name__ == "__main__":
    main()
